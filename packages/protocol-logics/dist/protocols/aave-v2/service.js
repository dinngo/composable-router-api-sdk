...
class Service extends common.Web3Toolkit {
    ...
    async getLendingPoolAddress() {
        if (!this.lendingPoolAddress) {
            ...
            const lendingPoolAddressProvider = contracts_1.LendingPoolAddressesProvider__factory.connect(lendingPoolAddressProviderAddress, this.provider);
    ...
    async getAssetAddresses() {
        if (!this.assetAddresses) {
            ...
            const lendingPool = contracts_1.LendingPool__factory.connect(lendingPoolAddress, this.provider);
    ...
    async getDebtTokenAddress(asset, interestRateMode) {
        const { stableDebtTokenAddress, variableDebtTokenAddress } = await this.protocolDataProvider.getReserveTokensAddresses(asset.address);
