...
let PermitTokenLogic = class PermitTokenLogic extends core.Logic {
    async getPermitData(account, erc20Funds) {
        const details = [];
        ...
        const sigDeadline = (0, utils_1.getDeadline)(constants_1.PERMIT_SIG_DEADLINE);
        if (details.length === 1) {
    ...
    async build(fields, options) {
        ...
        let data;
        if ((0, utils_1.isPermitSingle)(permit)) {
            data = contracts_1.Permit2__factory.createInterface().encodeFunctionData('permit(address,((address,uint160,uint48,uint48),address,uint256),bytes)', [account, permit, sig]);
        }
        else {
            data = contracts_1.Permit2__factory.createInterface().encodeFunctionData('permit(address,((address,uint160,uint48,uint48)[],address,uint256),bytes)', [account, permit, sig]);
        }
