...
class Service extends common.Web3Toolkit {
    get poolDataProvider() {
        return contracts_1.PoolDataProvider__factory.connect((0, config_1.getContractAddress)(this.chainId, 'PoolDataProvider'), this.provider);
    }
    async getPoolAddress() {
        if (!this.PoolAddress) {
            const PoolAddressProviderAddress = await this.poolDataProvider.ADDRESSES_PROVIDER();
            const PoolAddressProvider = contracts_1.PoolAddressesProvider__factory.connect(PoolAddressProviderAddress, this.provider);
    ...
    async getAssetAddresses() {
        if (!this.assetAddresses) {
            ...
            const Pool = contracts_1.Pool__factory.connect(PoolAddress, this.provider);
            const assetAddresses = await Pool.getReservesList();
            const iface = contracts_1.PoolDataProvider__factory.createInterface();
            const calls = assetAddresses.map((assetAddress) => ({
                target: (0, config_1.getContractAddress)(this.chainId, 'PoolDataProvider'),
    ...
    async toAsset(aToken) {
        const contract = contracts_1.AToken__factory.connect(aToken.address, this.provider);
    ...
    async isDelegationApproved(account, delegateeAddress, assetAmount, interestRateMode) {
        ...
        const debtToken = contracts_1.DebtTokenBase__factory.connect(debtTokenAddress, this.provider);
